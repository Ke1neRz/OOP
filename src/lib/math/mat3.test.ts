...
test("randomized sanity: transformPoint matches manual steps for many deterministic cases", () => {
  for (let i = 0; i < 30; i++) {
    const tx = (i % 7) - 3;
    const ty = ((i * 3) % 11) - 5;
    const angle = ((i * 0.37) % (2 * Math.PI)) - Math.PI;
...
test("multiply with identity should return original", () => {
  const I = mat3.identity();
  const m = mat3.fromTransform(5, 10, Math.PI / 3, 2, 3);

  expectMatCloseTo(mat3.multiply(m, I), m);
  expectMatCloseTo(mat3.multiply(I, m), m);
});

test("M * M^-1 should be identity", () => {
  const cases = [
    mat3.translate(10, 20),
    mat3.scale(2, 3),
    mat3.rotate(Math.PI / 4),
    mat3.fromTransform(5, 7, 0.3, 1.5, 2),
  ];
  ...
  for (const M of cases) {
    ...
    expectMatCloseTo(identity, mat3.identity(), 1e-8);
  }
});
