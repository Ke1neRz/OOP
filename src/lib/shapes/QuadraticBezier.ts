 ...
 * Defined by three control points: start, control, end
 * Formula: P(t) = (1-t)²*P0 + 2*(1-t)*t*P1 + t²*P2, where t ∈ [0, 1]
 */
export class QuadraticBezier extends Shape {
  // Control points in local coordinates (relative to center)
  x0: number; // Start point
  y0: number;
  x1: number; // Control point
  y1: number;
  x2: number; // End point
  y2: number;
  ...
  private evaluatePoint(t: number): { x: number; y: number } {
    ...
    return {
      x: a * this.x0 + b * this.x1 + c * this.x2,
      y: a * this.y0 + b * this.y1 + c * this.y2,
    };
  }
  ...
  getLocalBounds(): Bounds {
    const xs = [this.x0, this.x1, this.x2];
    const ys = [this.y0, this.y1, this.y2];
